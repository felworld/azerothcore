...
void WorldConfig::BuildConfigCache()
{
    ...
    // Controls dead-ghost (corpse run) movespeed rate.
    SetConfigValue<float>(RATE_MOVESPEED_GHOST, "Rate.MoveSpeed.Ghost", 1.0f, ConfigValueCache::Reloadable::Yes, [](float const& value) { return value >= 0.0f; }, ">= 0");
    ...
    SetConfigValue<uint32>(CONFIG_DUALSPEC_COST, "DualSpecCost", 10000000, ConfigValueCache::Reloadable::Yes, [](uint32 const& value) { return value <= MAX_MONEY_AMOUNT; }, "<= MAX_MONEY_AMOUNT");
    ...
    SetConfigValue<uint32>(CONFIG_FELWORLD_EVENTS_RETENTION_DAYS, "Felworld.Events.RetentionDays", 30);
